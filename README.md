...
> ✅ 包内已含 Node.js，无需安装任何东西。解压即用。
> 💡 首次启动会自动拉起 `dsh web` 服务（首次会从 npm 下载官方 `@deepseek-ai/dsh` 包，可能需要等十几秒并联网一次）。
...
| 现象 | 排查 |
| --- | --- |
| 窗口空白 / 打不开 | 先看 `dsh-server.log`，多半是缺 `DEEPSEEK_API_KEY` 或首次下载包失败（需联网）。 |
