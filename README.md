...
- 🔄 **自动更新（v1.0.3）**：启动时自动检查 npm 版本，发现新版本一键 `npm install` 即可更新引擎，无需重新下载 zip。
...
见 `agent/src/build.md`。需要本机有 Python 3.13+ 和 Node.js 22+。
...
启动时如果检测到 npm 上有新版本的 dsh 引擎，加载窗口会显示「发现新版本」提示，点击「更新引擎」即可一键 `npm install`，完成后自动重启服务。也可以跳过，不影响正常使用。

### 4. 关闭与退出

- 关闭窗口即退出桌面版；`dsh web` 服务进程会保留，下次启动更快。
- 想彻底退出服务：运行 `agent/src/dsh-quit.py`（需本机 Python）。

---

## 🔧 排错

启动器日志：`dsh-desktop.log`
dsh web 服务日志：`dsh-server.log`
...
| 更新失败 | 检查网络连接；或手动在 `app` 目录执行 `npm install`。不影响正常使用。 |
...
- 🔄 **自动更新**：启动时自动检查 npm 上 dsh 引擎版本，发现新版本时提示一键更新
- 📦 更新只需 npm install，无需重新下载 zip 包
- 💾 24 小时内版本检查结果缓存，不重复联网
...
├── .env.example                  环境变量模板
├── assets/
│   └── deepseek-whale.ico        DeepSeek 官方蓝鲸图标
├── agent/
│   ├── README.md                 面向用户的详细文档
│   ├── LICENSE                   MIT
│   ├── PUBLISH.md                发布指南
